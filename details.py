...
class DetailsAdd:
    def __init__(self, root):
        ...
        lblimg4 = Label(self.root, image=self.photoimage4, bg="navy", borderwidth=0)
        ...
        DataFrameRight.place(x=0, y=50, width=550, height=460)

        # ===========Images room===============
        img1 = Image.open("images/single22.jpeg")
        img1 = img1.resize((250, 130), Image.LANCZOS)
        self.photoImg1 = ImageTk.PhotoImage(img1)
        l1 = Label(DataFrameRight, image=self.photoImg1, borderwidth=0)
        l1.place(x=10, y=1)

        img2 = Image.open("images/double22.jpeg")
        img2 = img2.resize((250, 130), Image.LANCZOS)
        self.photoImg2 = ImageTk.PhotoImage(img2)
        l2 = Label(DataFrameRight, image=self.photoImg2, borderwidth=0)
        l2.place(x=265, y=1)

        img3 = Image.open("images/luxury22.jpeg")
        img3 = img3.resize((250, 130), Image.LANCZOS)
        self.photoImg3 = ImageTk.PhotoImage(img3)
        l1 = Label(DataFrameRight, image=self.photoImg3, borderwidth=0)
        l1.place(x=10, y=135)

        img4 = Image.open("images/luuxury222.jpg")
        img4 = img4.resize((250, 130), Image.LANCZOS)
        self.photoImg4 = ImageTk.PhotoImage(img4)
        l2 = Label(DataFrameRight, image=self.photoImg4, borderwidth=0)
        l2.place(x=265, y=135)

        # ================labels & entry=================
        l_floor = Label(
            DataFrameRight, text="Floor", fg="black", font=("times new roman", 15)
        )
        l_floor.place(x=0, y=280)

        self.floor_add_var = StringVar()
        entry_Floor = ttk.Entry(
            DataFrameRight,
            textvariable=self.floor_add_var,
            width=15,
            font=("times new roman", 15),
        )
        entry_Floor.place(x=135, y=280)

        l_roomNo = Label(
            DataFrameRight, text="Room No", fg="black", font=("times new roman", 15)
        )
        l_roomNo.place(x=0, y=315)

        self.room_add_var = StringVar()
        entry_Medicine = ttk.Entry(
            DataFrameRight,
            textvariable=self.room_add_var,
            width=15,
            font=("times new roman", 15),
        )
        entry_Medicine.place(x=135, y=315)

        l_roomType = Label(
            DataFrameRight, text="Room Type", fg="black", font=("times new roman", 15)
        )
        l_roomType.place(x=0, y=355)

        self.type_room = StringVar()
        roomType_combo = ttk.Combobox(
            DataFrameRight,
            width=13,
            textvariable=self.type_room,
            font=("times new roman", 15),
            state="readonly",
        )
        roomType_combo["values"] = ("Select Option", "Single", "Double", "Luxury")
        roomType_combo.place(x=135, y=355)
        ...
        side_frame.place(x=600, y=50, width=600, height=460)
        ...
        # ================================== Room Add button=============================================================
        down_frame = Frame(DataFrameRight, bd=0, relief=RIDGE)
        down_frame.place(x=10, y=390, width=530, height=40)
        ...
        add_btn.grid(row=0, column=0, pady=1)

        update_btn = Button(
            down_frame,
            text="UPDATE",
            command=self.room_update,
            font=("arial", 12, "bold"),
            width=12,
            fg="gold",
            bg="navy",
        ...
        delete_btn.grid(row=0, column=3, pady=1)

        clear_btn = Button(
            down_frame,
            text="CLEAR",
            command=self.clear_room,
            font=("arial", 12, "bold"),
            width=12,
            fg="gold",
            bg="navy",
        )
        clear_btn.grid(row=0, column=4, pady=1)
    ...
    def room_update(self):
        if self.floor_add_var.get() == "" or self.room_add_var.get() == "":
            ...
            try:
                ...
                conn.close()
                messagebox.showinfo(
                    "Success", "Data Successfully updated", parent=self.root
                )
